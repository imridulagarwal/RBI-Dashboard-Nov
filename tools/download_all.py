...
def _infer_year_month(link_text: str, href: str, context_text: str):
    # Try link text first (e.g., "September 2025")
    pool = " ".join([link_text or "", context_text or "", href or ""])
    # prefer “Month YYYY”
    ...
    if m:
        ...
        if mn: return yr, MONTHS[mn]
    # try in filename like ATMAUGUST2025..., ATMSEPTEMBER2025...
    m = re.search(r"([A-Za-z]{3,9})(\d{4})", href, flags=re.I)
    if m:
        mn = _norm_month(m.group(1))
        yr = int(m.group(2))
        if mn: return yr, MONTHS[mn]
    # as a last fallback, return None; emit_json can still be called with explicit inputs later
    return None, None
    ...
    with sync_playwright() as p:
        ...
        for a in anchors:
            ...
            if href.lower().endswith((".xls", ".xlsx")):
                ...
                # some context: parent/sibling text sometimes contains Month YYYY
                context_text = " ".join((a.parent.get_text(strip=True) if a.parent else "", a.next_sibling or ""))
